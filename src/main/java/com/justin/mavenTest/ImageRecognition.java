...
public class ImageRecognition {
	...
	public FrameEntiey getFrameEntiey() {
		return frameEntiey;
	}

	public void setFrameEntiey(FrameEntiey frameEntiey) {
	...
	public boolean matchImage(Mat templateImage, Mat originalImage) {
		...
		// 当匹配后的特征点大于等于 5 个，则认为模板图在原图中，该值可以自行调整
		System.out.println("*****************************************************");
		if (matchesPointCount >= 5) {
			Tools.println("匹配点数:" + matchesPointCount);
			System.out.println("*****************************************************");
			...
			/**
			 * 透视变换(Perspective Transformation)是将图片投影到一个新的视平面(Viewing
			 * Plane)，也称作投影映射(Projective Mapping)。
			 */
			...
			String fileName = Tools.getDateTime();
			Highgui.imwrite(frameEntiey.getModelPath()+"\\tempImage\\" + fileName + ".jpg", matchOutput);
			// Highgui.imwrite("E:/proc2.jpg", originalImage);
			return true;
		} else {
			// Tools.println("模板图不在原图中！");
			Tools.println("匹配点数:" + matchesPointCount);
			System.out.println("*****************************************************");
			return false;
		}
